...
st.set_page_config(page_title="Target Input - ExoX", page_icon=":dart:", layout="wide")
...
if input_method == "Gaia DR3 IDs":
    ...
    gaia_text = st.text_area("Gaia DR3 IDs", placeholder="1234567890123456789\n9876543210987654321", height=150, label_visibility="collapsed", key="gaia_input")
    if gaia_text:
    ...
    st.caption("One ID per line, or comma-separated")
    tic_text = st.text_area("TIC IDs", placeholder="118809859\n123456789", height=150, label_visibility="collapsed", key="tic_input")
    if tic_text:
    ...
    coord_text = st.text_area("Coordinates", placeholder="231.9278 2.5977\n82.7072 -67.0986", height=150, label_visibility="collapsed", key="coord_input")
    if coord_text:
...
if target_ids:
    ...
    st.dataframe(pd.DataFrame({"target_id": target_ids, "index": range(1, len(target_ids)+1)}).set_index("index"), use_container_width=True)
else:
    st.info("Enter target IDs above or upload a CSV file.")

user_run_name = st.text_input("Run Name (optional)", value=f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}", key="user_run_name")
...
if st.button("Save Targets & Continue", type="primary", use_container_width=True):
    if not target_ids:
        ...
        st.markdown("Click below to proceed to Gaia DR3 Filter:")
